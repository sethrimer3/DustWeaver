...
import { dispatchRoomToWorker, isRoomPendingWithWorker } from './roomPreparationWorkerManager';
...
export function scheduleRoomPreloads(
  ...
  function processNext(deadline: IdleDeadline): void {
    ...
    if (radius >= 1) {
      ...
      if (estimatedCostMs > threshold) {
        const dispatched = dispatchRoomToWorker(roomId, room, cache, isDebugMode);
        if (dispatched) {
          if (isDebugMode) {
            console.log(
  ...
  return {
    ...
    prioritize(roomId: string): void {
      ...
      if (isRoomPendingWithWorker(roomId)) {
        if (isDebugMode) {
