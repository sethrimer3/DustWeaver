...
export function editorRoomDataToRoomDef(data: EditorRoomData): RoomDef {
  ...
  const enemies: RoomEnemyDef[] = data.enemies.map(e => {
    ...
    return {
      ...
      isDustWeaverArchitectFlag: e.isDustWeaverArchitectFlag ?? 0,
      isDustWeaverArchitectLargeFlag: e.isDustWeaverArchitectLargeFlag ?? 0,
  ...
  return {
    ...
    crumbleBlocks: (data.crumbleBlocks ?? []).map(b => ({
      ...
      roughStairOrientation: b.roughStairOrientation,
