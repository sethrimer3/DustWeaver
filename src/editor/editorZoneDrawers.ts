...
/**
 * Editor-only preview length (in blocks) for the direction indicator drawn
 * off a laser emitter. The real beam's length is only known at room-load
 * time (resolved by a wall raycast — see loadRoomHazards.ts), so this is
 * purely a fixed-length visual affordance, not a prediction of the actual
 * in-game beam length.
 */
const LASER_EDITOR_PREVIEW_LENGTH_BLOCKS = 4;
...
export function drawEditorLasers(
  ...
  for (const l of (room.lasers ?? [])) {
    ...
    const fillAlpha = sel ? 0.85 : 0.6;
    ctx.fillStyle = `rgba(255,90,0,${fillAlpha})`;
    ctx.strokeStyle = `rgba(255,220,180,${sel ? 1.0 : 0.75})`;
    ctx.lineWidth = sel ? 2 : 1;
    ...
    // Direction preview line — fixed length, purely a visual affordance (see
    // LASER_EDITOR_PREVIEW_LENGTH_BLOCKS's doc comment above).
    const previewLenPx = LASER_EDITOR_PREVIEW_LENGTH_BLOCKS * BLOCK_SIZE_SMALL * zoom;
    let dirX = 0, dirY = 0;
    switch (l.direction) {
    ...
    ctx.save();
    ctx.strokeStyle = `rgba(255,140,30,${sel ? 0.9 : 0.5})`;
    ctx.lineWidth = Math.max(1, zoom * 0.6);
    ctx.setLineDash([zoom * 2, zoom * 1.5]);
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + dirX * previewLenPx, cy + dirY * previewLenPx);
    ctx.stroke();
    ctx.restore();
  }
}
