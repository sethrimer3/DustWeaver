...
import { dehydrateRoom, enemyFlagsToType } from '../levels/roomSchemaV2';
...
test('documented persistence contract survives the complete editor/save/runtime chain', () => {
  ...
  for (const key of EDITOR_ROOM_ELEMENT_COLLECTION_KEYS) {
    if (key === 'interiorWalls') continue; // Runtime import deliberately splits editable solid rectangles into tiles.
    const before = original[key] ?? [];
    const after = finalEditor[key] ?? [];
    assert.equal(after.length, before.length, key);
  }
  ...
  assert.equal(finalEditor.enemies.find(e => e.isGoldenMimicFlag === 1)?.isGoldenMimicYFlippedFlag, 1);
  assert.equal(finalEditor.enemies.find(e => e.isMomentumTurretFlag === 1)?.momentumTurretFacingIndex, 3);
});

test('corrupt saved enemy tags cannot hydrate as basic', () => {
