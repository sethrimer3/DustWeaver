...
function bfsNextDirection(
  ...
  // Clear visited buffer only in the relevant area to avoid full clear overhead.
  // We track which cells we wrote so we can un-mark them after.
  const visitedCells: number[] = [];

  let head = 0;
  let tail = 0;

  // BFS queue stores grid coords; direction arrays store how we got there.
  ...
  while (head < tail && !found) {
    ...
    for (let d = 0; d < 4; d++) {
      ...
      // Check if the neighbour cell is passable for this enemy size.
      ...
      // Track first step direction from start to neighbour.
...
/** Convert a committed grid X to the world-space center X for size variant. */
function gridToWorldX(gx: number, sizeIndex: number): number {
  return gx * BS + (sizeIndex === 0 ? BS * 0.5 : BS);
}

/** Convert a committed grid Y to the world-space center Y for size variant. */
  ...
  for (let ci = 0; ci < world.clusters.length; ci++) {
    ...
    if (cluster.gridBlockMoveTicks > 0) {
      ...
      if (cluster.gridBlockRepathCooldownTicks <= 0) {
        ...
        if (playerFound) {
          // Determine player's tile from world position.
          let targGX = Math.floor(playerXWorld / BS);
          let targGY = Math.floor(playerYWorld / BS);

          // For 2×2 enemies, shift target so the enemy center lines up near player.
          if (sizeIndex === 1) {
      ...
      if (ndx !== 0 || ndy !== 0) {
        ...
        // Verify the target cell is in-bounds and passable.
