/**
 * Falling Block simulation — tick logic, trigger detection, and wall-slot
 * management.
 *
 * Pure collision geometry helpers (shape contact tests, landing surface
 * detection) live in fallingBlockCollision.ts to keep this file focused on
...
import {
  isPlayerRestingOnGroupTop,
  playerContactsGroup,
  findContactingEnemyIndex,
  pointInGroupShape,
  findLandingSurface,
  LAND_OVERLAP_EPSILON,
} from './fallingBlockCollision';

// Re-export the group bound accessors so existing callers (e.g. fallingBlockRenderer)
// can continue importing them from this module without changes.
export {
  getFBGroupTopWorld,
  getFBGroupBottomWorld,
  getFBGroupLeftWorld,
