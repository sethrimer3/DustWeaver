 ...
 *
 * Draws a thin, fading STRAIGHT preview line from the player along the current
 * aim direction while the Bow arrow is assembling (Press/Holding). The Bow
 * Weave no longer has a draw-strength / ballistic arc, so the preview is always
 * a straight ray — there is no parabola, no gravity, and no per-tier curve. The
 * line is clipped at the first wall along the aim (using the same
 * `raycastWalls()` helper the real arrow uses) or at the Gold Dust maximum
 * outbound travel distance, whichever is closer.
 *
...
/**
 * Computes the straight aim-line end point: the player start point extended
 * along the unit aim direction by `maxRangeWorld`, clipped to the first wall
 * hit if any. Writes the end point into `out` and returns it. Pure and
 * allocation-free (writes into the caller-provided object).
 */
export function computeStraightBowAimEnd(
  ...
  const dirX = len > 1e-6 ? aimDirXWorld / len : 1;
  const dirY = len > 1e-6 ? aimDirYWorld / len : 0;
...
export class BowTrajectoryPreviewRenderer {
  ...
  render(
    ...
    const end = computeStraightBowAimEnd(
      this._end, playerXWorld, playerYWorld, dx, dy, BOW_PREVIEW_MAX_RANGE_WORLD, raycast,
    );

    const dustKind: ParticleKind = snapshot.currentWeaveDustKind;
    const colorHex = getDustDefinition(dustKind).colorHex;

    const x0 = playerXWorld * zoom + ox;
    const y0 = playerYWorld * zoom + oy;
