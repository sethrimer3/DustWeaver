 ...
 *   5. Scale the shading intensity by `EDGE_SHADING_STRENGTH` (1.0 = default; raise for
 ...
 *
 * This baked shading only ever darkens texture (multiply, never additive), so
 * it cannot exceed a pixel's original value. The single production exposed-edge
 * *highlight* is drawn separately, every frame, by `surfaceEdgeOverlay.ts` from
 * the authoritative `SurfaceExposureMap` — see that module's doc comment for why.
 *
 ...
 * visually stuck.
 */
export const EDGE_SHADING_VERSION = 3;
...
const _EDGE_INNER_CORNER_DARKEN = -0.05;
...
export function applyOrganicEdgeShading(
  ...
  // ── Pass 1: multiply-based darkening ──────────────────────────────────────
