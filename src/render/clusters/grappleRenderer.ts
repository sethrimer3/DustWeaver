 ...
 *
 * One-shot FX helpers (fail beam, empty FX, zip impact, recharge ring,
 * zip-jump ready ring) have been extracted to grappleFxRenderers.ts.
 */
...
export function renderGrapple(ctx: CanvasRenderingContext2D, snapshot: WorldSnapshot, offsetXPx: number, offsetYPx: number, scalePx: number, isDebugMode = false): void {
  ...
  // Shown during zip travel and while the zip-jump timing window is open.
  if (snapshot.isGrappleZipActiveFlag === 1 && snapshot.isGrappleActiveFlag === 1 &&
      (snapshot.isGrappleStuckFlag === 0 || snapshot.isZipJumpWindowOpenFlag === 1)) {
