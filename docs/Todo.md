...
- [ ] Preserve the visual-map position of a room's opposite edge when resizing from the top or left. Repro: remove rows from the **top** with the Room Dimensions edge controls; because `heightBlocks` changes while `mapY` remains fixed, the room rectangle appears to move upward instead of losing space from its top. Top-edge shrink must keep the old bottom map coordinate fixed and move only the top edge downward; top-edge growth must extend upward. In `src/editor/editorRoomResize.ts::applyEdgeResize`, adjust `room.mapY` by the inverse of the effective/clamped top delta (`mapY -= clampedDelta`) in the same atomic resize operation, while retaining the existing local-content shift so surviving content remains at the same map-world coordinates. Apply the analogous invariant to left-edge resizing with `room.mapX -= clampedDelta`; bottom/right resizing must not change `mapX`/`mapY`. Do not double-shift content, break transition/door alignment, or desynchronize Zone Map, Itemized Map/map sketch, undo/redo, autosave, room JSON, or campaign export. Relevant types/persistence: `EditorRoomData.mapX/mapY` in `src/editor/editorElementTypes.ts`, visual-map placement in `src/editor/editorVisualMap*.ts`, and room registry/serialization paths. Extend `src/tests/editorRoomResizeBulk.test.ts` (or add a focused test) for ±1/±5 top and left changes, minimum-size-clamped shrink, opposite-edge invariance (`mapY + heightBlocks` / `mapX + widthBlocks`), bottom/right position stability, surviving-content world-position invariance, and one-step undo restoring dimensions, map coordinates, and content. Run targeted resize/history tests, then `npm run build`, `npm run lint`, and `npm test`.
