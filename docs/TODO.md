...
- [ ] Editor UI redesign, part 2/2 (still deferred): in-memory session-persisted panel/sidebar-visibility state (survives editor close/reopen, not app restart) beyond the layers panel's existing Phase 6 persistence, per-sidebar hide arrows with inward-pointing edge reveal tabs, and replacing the existing hardcoded left-260px pointer-exclusion hit check in `editorController.ts` (`EDITOR_PANEL_WIDTH_CSS_PX`) with shared hit-test logic that accounts for both sidebars' visibility/width and their reveal tabs — note the right sidebar currently has no pointer-exclusion at all, so clicks under it can fall through to the canvas.
